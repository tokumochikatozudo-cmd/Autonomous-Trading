...
class CryptoBrokerClient:
    ...
    def _init_exchange(self):
        ...
        # We will handle Bybit domain fallback dynamically in check_health()
    ...
    async def check_health(self) -> bool:
        ...
        for domain in domains_to_try:
            ...
            try:
                # We do a direct quick fetch_time with 1 retry to test the domain quickly
                await self._safe_execute(self.exchange.fetch_time, max_retries=1, base_delay=0.5)
                
                # If we get here, the domain works! 
                # Now explicitly synchronize CCXT's internal clock with the exchange
                # to prevent the "timestamp in the future" Error 10002.
                try:
                    await self.exchange.load_time_difference()
                    logger.info("Synchronized internal clock with exchange.")
                except Exception as time_e:
                    logger.warning(f"Could not load time difference: {time_e}")
