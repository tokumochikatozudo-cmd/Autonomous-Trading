...
class TradingBridgeOrchestrator:
    ...
    async def fetch_historical_data(self) -> pd.DataFrame:
        """Fetch 500 hourly OHLCV bars from Bybit for HMM training."""
        logger.info("Fetching historical data for HMM training...")
        symbol = CONFIG['execution']['symbol']
        ohlcv = await self.broker._safe_execute(
            self.broker.exchange.fetch_ohlcv, symbol, '1h', limit=500
        )
